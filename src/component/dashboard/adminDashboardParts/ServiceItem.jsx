    ...
    return (
        <li>
            ...
            <Accordion.Item eventKey={props.eKey} >
                ...
                {
                    state.services.map( (item, index) => <span className={styles['service-item']} key={index}>
                        {
                            item.serviceTitle
                        }
                        <i title='حذف' role="button" onClick={()=>{deleteService(item.id, 1)}} className="fa fa-minus text-danger" aria-hidden="true"></i>
                    </span>
                    )
                }
                </div>
